...
export const teacherLogin = async (req, res) => {
  ...
  try {
    // First try skydek_DB for teachers
    let rows = await query('SELECT * FROM users WHERE email = ? AND role = "teacher"', [email], 'skydek_DB');
    
    // If no teacher found in skydek_DB, try railway database
    if (!rows || rows.length === 0) {
      rows = await query('SELECT * FROM users WHERE email = ?', [email], 'railway');
    }
